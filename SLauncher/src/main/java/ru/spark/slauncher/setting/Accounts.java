...
public final class Accounts {
    ...
    private static ObjectProperty<Account> selectedAccount = new SimpleObjectProperty<Account>(Accounts.class, "selectedAccount") {
        {
            accounts.addListener(FXUtils.onInvalidating(this::invalidated));
        }
    ...
    private static void updateAccountStorages() {
        ...
        if (!initialized)
            return;
        // update storage
        ConfigHolder.config().getAccountStorages().setAll(accounts.stream().map(Accounts::getAccountStorage).collect(toList()));
    }
    ...
    private static AuthlibInjectorServer getOrCreateAuthlibInjectorServer(String url) {
        return ConfigHolder.config().getAuthlibInjectorServers().stream()
    ...
    private static void removeDanglingAuthlibInjectorAccounts() {
        accounts.stream()
                ...
                .filter(it -> !ConfigHolder.config().getAuthlibInjectorServers().contains(it.getServer()))
                .collect(toList())
                .forEach(accounts::remove);
    }

    public static String getLocalizedLoginTypeName(AccountFactory<?> factory) {
        return I18n.i18n(Optional.ofNullable(unlocalizedLoginTypeNames.get(factory))
                .orElseThrow(() -> new IllegalArgumentException("Unrecognized account factory")));
    }
    // ====
}
